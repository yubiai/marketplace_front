...
const Sales = () => {
  ...
  const { data, isLoading, isError} = useFetch(
    global && global.profile && global.profile.eth_address
      ? `/orders/seller/${global.profile.eth_address}?page=${global.pageIndex}&size=5&network=${networkType}`
      : null,
        ...
        >
          ...
          {data &&
            ...
            data.items.map((item, i) => {
              return <OrderCardSeller yubiaiContract={yubiaiContract.yubiaiArbitrable} abiYubiai={yubiaiArbitrable} order={item} network={networkType} key={i} t={t} />
            })}
